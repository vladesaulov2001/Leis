...
void cMenu::InitializeMenu()
{
	...
	if( M_EntryPos == 0 )
	{
		...
		AddSubMenu( CFG_AIM_BONE , &cvar.weapon_settings[cvar.wpn].aim_bone , 2 , 8 , 1 );
...
void cMenu::DrawMenu( int x , int y , bool center )
{
	...
	g_Draw.DrawBox( x , y , menu_head_width , ENTRY_HEIGHT  , Color( 255 , 0 , 255 , 255 ) );
	g_Draw.FillArea( x + 1 , y + 1 , menu_head_width - 2 , ENTRY_HEIGHT - 2 , Color( 255 , 0 , 255 , 80 ) );
	...
	{
		g_FontMenu.DrawString( x + ( menu_head_width / 2 ) , y , Color( 0 , 255 , 0 , 255 ) , true , CFG_LEIS_LOADED );
	}
	else
	{
		dwLastLoadTime = 0;
	...
	{
		g_FontMenu.DrawString( x + ( menu_head_width / 2 ) , y , Color( 0 , 255 , 0 , 255 ) , true , CFG_LEIS_SAVED );
	}
	else
	{
		dwLastSaveTime = 0;
	...
	{
		g_FontMenu.DrawString( x + ( menu_head_width / 2 ) , y , Color( 255 , 255 , 255 , 255 ) , true , CFG_HACK_NAME );
	}

	y += ENTRY_HEIGHT + MENU_PADDING_TOP;

	// меню с функциями чита
	g_Draw.DrawBox( x , y , MENU_ENTRY_WIDTH , ENTRY_HEIGHT * M_EntrySize + 2 , Color( 255 , 0 , 255 , 255 ) );
	g_Draw.FillArea( x + 1 , y + 1 , MENU_ENTRY_WIDTH - 2 , ENTRY_HEIGHT * M_EntrySize , Color( 255 , 0 , 255 , 80 ) );

	for ( int i = 0; i < M_EntrySize; i++ )
	{
		if ( i == M_EntryPos && !SubMenu )
		{
			g_Draw.FillArea( x + 1 , y + 1 + ENTRY_HEIGHT * i , MENU_ENTRY_WIDTH - 2 , ENTRY_HEIGHT , Color( 255 , 255 , 255 , 50 ) );
		}
	...
	// меню с кварами чита
	g_Draw.DrawBox( subm_x , y , SUBM_ENTRY_WIDTH , ENTRY_HEIGHT * S_EntrySize + 2 , Color( 255 , 0 , 255 , 255 ) );
	g_Draw.FillArea( subm_x + 1 , y + 1 , SUBM_ENTRY_WIDTH - 2 , ENTRY_HEIGHT * S_EntrySize , Color( 255 , 0 , 255 , 80 ) );
	...
	for ( int i = 0; i < S_EntrySize; i++ )
	{
		...
		r1 = 0; g1 = 255; b1 = 0; // on
		r2 = 255; g2 = 0; b2 = 0; // off
		r3 = 255; g3 = 255; b3 = 250; // value
		...
		if ( i == S_EntryPos && SubMenu )
		{
			...
			g_Draw.FillArea( subm_x + 1 , y + 1 + ENTRY_HEIGHT * i , SUBM_ENTRY_WIDTH - 2 , ENTRY_HEIGHT , Color( 255 , 255 , 255 , 50 ) );
		}
